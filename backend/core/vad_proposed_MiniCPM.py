...
def main():
    ...
    if cfg.anomaly_detect:
        ...
        keyword_kfs_features = {}
        with torch.no_grad():
            for keyword in keyword_list:
                texts = clip.tokenize([keyword]).to(device)
                features = clip_model.encode_text(texts).float()
                features /= features.norm(dim=-1, keepdim=True)
                keyword_kfs_features[keyword] = features

        if cfg.class_adaption:
            # If class_adaption is True, embedding is constant for all keywords (based on type_list)
            precomputed_wa_embedding = make_text_embedding(clip_model, device, text=None, type_list=cfg.type_list,
                                                          class_adaption=True, template_adaption=cfg.template_adaption)
        else:
            precomputed_wa_embeddings = {}
            for keyword in keyword_list:
                precomputed_wa_embeddings[keyword] = make_text_embedding(clip_model, device, text=keyword, type_list=None,
                                                                         class_adaption=False, template_adaption=cfg.template_adaption)
        # -------------------------------------------------------------------------
            ...
            for i, video_path in progress_bar(enumerate(video_paths), total=len(video_paths)):
                ...
                for cp in progress_bar(cp_loader, total=len(cp_loader)):
                    ...
                    for k_i, keyword in enumerate(keyword_list):
                        ...
                        if cfg.class_adaption:
                            text_embedding = precomputed_wa_embedding
                        else:
                            text_embedding = precomputed_wa_embeddings[keyword]

                        kfs_feat = keyword_kfs_features[keyword]

                        indice = kfs.call_function(cp, keyword, text_features=kfs_feat)
                        ...
                        grid_image = grid_generation(cfg, image_paths, keyword, clip_model, device, text_features=kfs_feat)
